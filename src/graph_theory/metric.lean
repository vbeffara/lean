...
namespace simple_graph
    ...
    lemma shortest_path (G : simple_graph V) [connected_graph G] (x y) : ∃ p : walk G x y, length p = dist G x y
    ...
    noncomputable instance has_dist {V : Type} (G : simple_graph V) [connected_graph G] : has_dist V
        := ⟨λ x y, simple_graph.dist G x y⟩

    noncomputable instance metric_space {V : Type} (G : simple_graph V) [connected_graph G] : metric_space V
