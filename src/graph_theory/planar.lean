...
namespace simple_graph
    ...
    def plane : simple_graph (ℤ × ℤ) := ⟨plane.adj,plane.adj_symm⟩
    ...
    namespace plane
        ...
        def flip : plane →g plane := {
            to_fun := flip',
            map_rel' := by { rintros ⟨x,y⟩ ⟨x',y'⟩ h, cases h, { right, exact and.symm h }, { left, exact and.symm h } }
        }
        ...
        lemma connected_plane : connected plane
            | (x,y) (x',y') := linked.trans horiz_path vert_path
    end plane
