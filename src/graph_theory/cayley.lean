...
namespace cayley
    ...
    def Cay : Graph := { V := G, adj := adj S, sym := @adj_symm _ _ S }
    ...
    noncomputable def word_dist : G -> G -> ℕ := @graph.dist (Cay S) _
...
namespace cayley
    ...
    lemma lipschitz : ∃ K : ℕ, ∀ x y : G, word_dist S2 x y <= K * word_dist S1 x y
        := by { obtain K := max_of_nonempty (nonempty.image S1.nem _), use K, 
