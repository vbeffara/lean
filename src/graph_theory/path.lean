...
namespace Graph
    ...
    structure path (G : Graph V) (x y) extends llist' V x y := (adj : llist.is_path G.adj l)

    namespace path
        variables {G : Graph V} {x y z : V}

        instance : has_coe (path G x y) (llist' V x y) := ⟨path.to_llist'⟩

        def mem (v : V) (p : path G x y) := v ∈ p.l
    ...
    @[ext] structure  spath (G : Graph V) (x y) extends path G x y 
