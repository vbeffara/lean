...
namespace llist section
    ...
    @[simp] def compat (l₁ l₂ : llist V) := last l₁ = first l₂
    ...
    @[simp] lemma concat_first (h) : first (concat l l' h) = first l              := by { induction l; finish [h.symm] }
    ...
    lemma concat_nodup (h) : nodup (concat l l' h) <-> nodup l ∧ nodup l' ∧ (∀ x, x ∈ l ∧ x ∈ l' -> x = first l') := by {
...
namespace llist' section open llist
    ...
    @[simp] lemma first  {l : llist' V x y}                     : l.l.first = x         := l.hx.symm
