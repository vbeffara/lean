...
begin
  ...
  simp only,
  set ff := (lp 2 f).to_Lp f,
  set gg := (lp 2 g).to_Lp g,
  have := @real_inner_le_norm _ _ ff gg,
  convert ← this using 1,
  simp only [snorm, snorm', Lp.norm_to_Lp, ennreal.bit0_eq_zero_iff, one_ne_zero,
    ennreal.bit0_eq_top_iff, ennreal.one_ne_top, ennreal.to_real_bit0, ennreal.one_to_real,
    ennreal.rpow_two, one_div, if_false],
  congr,
  { have := lintegral_coe_eq_integral (λ x, ∥f x∥₊^2) _,
    ...
    { split,
      ...
      { cases lp 2 f,
        ...
        simp [snorm, snorm'] at right ⊢,
        have := @ennreal.rpow_lt_top_of_nonneg _ 2 (by norm_num) right.ne_top,
        rw ← ennreal.rpow_mul at this,
        simp at this,
        exact this } } },
  { have := lintegral_coe_eq_integral (λ x, ∥g x∥₊^2) _,
    { simp at this,
      rw [this, ennreal.of_real_rpow_of_nonneg, ennreal.to_real_of_real],
      { simp_rw pow_two,
        congr' 1,
        convert integral_congr_ae ((lp 2 g).coe_fn_to_Lp.mul (lp 2 g).coe_fn_to_Lp).symm,
        funext,
        exact abs_mul_abs_self _},
      { apply real.rpow_nonneg_of_nonneg,
        apply integral_nonneg,
