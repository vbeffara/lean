...
section embedding
    ...
    lemma follow_edges_llist {z : G'} {l : llist G} {h} {hz : l ≠ llist.P l.head} :
    ...
    lemma follow_edges {z} {p : path G x y} {hz : p.l ≠ llist.P x} :
            z ∈ follow F p <-> ∃ e ∈ path.edges p, z ∈ (F.df e).l
        := by { convert follow_edges_llist F, convert hz, simp }
