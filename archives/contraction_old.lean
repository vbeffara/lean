...
namespace simple_graph
    ...
    def contraction_old (G : simple_graph V) (S : contraction_old.setup G) : simple_graph S.clusters
        := G / S.setoid

    notation G `/` S := contraction_old G S

    def is_contraction_old (G : simple_graph V) (G' : simple_graph V') : Prop
        := ∃ S : contraction_old.setup G', nonempty (G ≃g (G'/S))

    infix ` ≼co `:50 := is_contraction_old

    lemma contraction_iff : G ≼co G' ↔ G ≼c G' :=
    begin
        split,
        { rintro ⟨S,⟨⟨f,f',h₁,h₂⟩,h₃⟩⟩, simp [contraction_old] at h₃, let φ : V' -> V := f' ∘ quotient.mk', refine ⟨φ,_,_,_⟩,
        ...
        { rintro ⟨f,h₁,h₂,h₃⟩, subst h₃,
            ...
            let S : contraction_old.setup G' := ⟨g, λ x' y' h, h.2⟩, use S, letI : setoid V' := S.setoid,
            ...
            refine ⟨⟨⟨F,F',H₁,H₂⟩,_⟩⟩,
            simp [push,contraction_old,contraction_old.setup.setoid,quotient_graph,F], intros a b, split,
    ...
    namespace is_contraction_old
        open contraction_old

        @[refl] lemma refl : G ≼co G := ⟨setup.bot,⟨proj_bot⟩⟩

        lemma iso_left : G ≃g G' -> G' ≼co G'' -> G ≼co G''
        := by { simp_rw contraction_iff, exact is_contraction.iso_left }

        lemma le_left : H ≤ G -> G ≼co G' -> ∃ H' : simple_graph V', H ≼co H' ∧ H' ≤ G'
        := by { simp_rw contraction_iff, exact is_contraction.le_left }

        @[trans] lemma trans : G ≼co G' -> G' ≼co G'' -> G ≼co G'' :=
        by { simp_rw contraction_iff, exact is_contraction.trans }

        lemma select_left {P : V → Prop} : G ≼co G' -> ∃ P' : V' → Prop, select P G ≼co select P' G'
        := by { simp_rw contraction_iff, exact is_contraction.select_left }
    end is_contraction_old
end simple_graph
